...
union aligned_buffer {
  typedef aligner_type<sizeof(_Aligner<T>) - sizeof(T)> aligner_type_buffer;

  aligner_type_buffer m_aligner;
  char m_buf[sizeof(T)];

private:
  TCG_STATIC_ASSERT(sizeof(_Aligner<T>) - sizeof(T) ==
                    sizeof(_Aligner<aligner_type_buffer>) - sizeof(aligner_type_buffer));
};
